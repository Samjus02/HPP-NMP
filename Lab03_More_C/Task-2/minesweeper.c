...
int main(int argc, char const *argv[])
{
  ...
  // ALLOCATE MEMORY FOR A 2D ARRAY field WITH SIZE (n, m)
   ...
   /*
       YOUR CODE
  */
   field = (int**)malloc(n * sizeof(int*)); // Allocate memory for row pointers
   if (field == NULL) {
   ...
   for (int i = 0; i < n; i++) {
      field[i] = (int*)malloc(m * sizeof(int)); // Allocate memory for each row
      if (field[i] == NULL) {
   ...
   */
   for (int i = 0; i < n; i++) {
      free(field[i]); // Free each row
   }
   free(field); // Free the array of pointers
